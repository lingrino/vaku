...
async fn test_path_move() {
    ...
    let cases = vec![
        ...
        Case { src: "0/4/8", dst: "0/4/5", want_err: vec![], nil_src: true, nil_dst: false },
        Case {
            src: "error/read/inject",
            dst: "move/readerror",
            want_err: vec![
                ErrorKind::PathMove.into(),
    ...
    for tt in cases {
        for (psrc, pdst) in seeded_prefix_product().await {
            ...
            let res = clients.vaku.path_move(&src, &dst).await;
