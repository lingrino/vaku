...
async fn test_path_copy_all_versions() {
    ...
    let cases = vec![
        Case {
            src: "0/1",
            dst: "copy/allversions/0/1",
            want_err: vec![],
            nil_dst: false,
        },
        Case {
            src: "0/4/5",
            dst: "copy/allversions/different",
            want_err: vec![],
            nil_dst: false,
        },
        Case {
            src: "fake/nonexistent",
            dst: "copy/allversions/fake",
            want_err: vec![],
            nil_dst: true,
        },
        Case {
...
async fn test_path_copy_all_versions_with_deleted_versions() {
    ...
    for prefix in seeded_prefixes("multiversion").await {
        ...
        clients
            .clean
            .path_write(&src, Some(v1.clone()))
            .await
            .unwrap();
        clients
            .clean
            .path_write(&src, Some(v2.clone()))
            .await
            .unwrap();
        clients
            .clean
            .path_write(&src, Some(v3.clone()))
            .await
            .unwrap();
        ...
        clients
            .vaku
            .path_copy_all_versions(&src, &dst)
            .await
            .unwrap();
...
async fn test_path_copy_all_versions_with_destroyed() {
    ...
    for prefix in seeded_prefixes("destroyed").await {
        ...
        let d2 = clients
            ...
            .await
            .unwrap();
        assert!(d2.is_none_or(|m| m.is_empty()));
        let d3 = clients
            .clean
            .as_destination()
            .path_read_version(&dst, 3)
