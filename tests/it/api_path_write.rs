...
async fn test_path_write() {
    ...
    let cases = vec![
        ...
        Case {
            give: "nildata",
            data: None,
            want_err: vec![ErrorKind::PathWrite.into(), ErrorKind::NilData.into()],
            no_readback: true,
        },
        Case {
            give: "error/write/inject",
            data: mk(&[("0", "1")]),
            want_err: vec![ErrorKind::PathWrite.into(), ErrorKind::VaultWrite.into()],
            no_readback: true,
        },
        Case {
            give: MOUNTLESS,
    ...
    for tt in cases {
        for prefix in seeded_prefixes(tt.give).await {
            ...
            let res = clients.vaku.path_write(&p, tt.data.clone()).await;
            ...
            if !tt.no_readback {
                ...
                assert_eq!(read, tt.data, "readback mismatch for {}", tt.give);
            }
        }
    }
}
