...
impl Client {
    ...
    pub async fn path_destroy(&self, p: &str, versions: &[i64]) -> Result<(), Error> {
        if versions.is_empty() {
            return Err(Error::wrap("no versions provided", ErrorKind::PathDestroy, None));
        }

        let (vault_path, _) =
            rewrite_path(self.src().mount_provider.as_ref(), p, VaultOp::Destroy)
                .await
                .map_err(|e| Error::wrap(p, ErrorKind::PathDestroy, Some(Box::new(e))))?;

        let mut body = Map::new();
        body.insert(
            KV2_VERSIONS.to_string(),
            json!(versions.iter().copied().collect::<Vec<i64>>()),
        );

        self.src().logical.write(&vault_path, Value::Object(body)).await.map_err(|e| {
            Error::wrap(
                p,
